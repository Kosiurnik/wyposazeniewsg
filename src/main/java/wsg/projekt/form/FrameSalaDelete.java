...
public class FrameSalaDelete extends JDialog {

	private static final long serialVersionUID = 1L;
	private JPanel contentPane;
	
	public FrameSalaDelete(final EntitySala sala) {
		setModal(true);
		setResizable(false);
		setTitle("Usuń salę");
		setAlwaysOnTop(true);
		...
		JLabel lblDasdad = new JLabel("Sala o numerze "+sala.getNumerSali());
		...
		btnAccept.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				try(AppEntityManager em = new AppEntityManager()){
						...
						em.getEntityManager().remove(em.getEntityManager().find(EntitySala.class, sala.getSalaID()));
