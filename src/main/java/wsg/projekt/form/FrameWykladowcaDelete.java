...
public class FrameWykladowcaDelete extends JDialog {

	private static final long serialVersionUID = 1L;
	private JPanel contentPane;
	
	public FrameWykladowcaDelete(final EntityWykladowca wykladowca) {
		setModal(true);
		setResizable(false);
		setTitle("Usuń wykładowcę");
		setAlwaysOnTop(true);
		...
		JLabel lblDasdad = new JLabel("Wykładowca: "+wykladowca.getImie()+" "+wykladowca.getNazwisko());
		...
		btnAccept.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				try(AppEntityManager em = new AppEntityManager()){
						...
						em.getEntityManager().remove(em.getEntityManager().find(EntityWykladowca.class, wykladowca.getWykladowcaID()));
