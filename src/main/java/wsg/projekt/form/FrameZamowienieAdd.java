...
public class FrameZamowienieAdd extends JDialog {
	...
	public FrameZamowienieAdd() {
		...
		//podgląd okienek w Eclipse świruje, kiedy widzi pola zawierające obiekty niestandardowego typu...
		...
		ComboBoxFilterDecorator<EntityWykladowca> dekoratorWykladowca = ComboBoxFilterDecorator.decorate(cbWykladowca,
                ComboRendererWykladowca::getWykladowcaText,
                FrameZamowienieAdd::wykladowcaFilter);
		...
		ComboBoxFilterDecorator<EntitySala> dekoratorSala = ComboBoxFilterDecorator.decorate(cbSala,
                ComboRendererSala::getSalaText,
                FrameZamowienieAdd::salaFilter);
		...
		ComboBoxFilterDecorator<EntitySprzet> dekoratorSprzet = ComboBoxFilterDecorator.decorate(cbSprzet,
                ComboRendererSprzet::getSprzetText,
                FrameZamowienieAdd::sprzetFilter);
		...
		tableSprzet.getCellEditor(0,1).addCellEditorListener(new CellEditorListener(){
			...
			public void editingStopped(ChangeEvent arg0) {
				System.out.println("Próba zmiany wertości z: "+tableSprzet.getValueAt(tableSprzet.getSelectedRow(), tableSprzet.getSelectedColumn())+" na "+tableSprzet.getCellEditor(tableSprzet.getSelectedRow(), tableSprzet.getSelectedColumn()).getCellEditorValue().toString()+" w: wiersz:"+tableSprzet.getSelectedRow()+"|kolumna:"+tableSprzet.getSelectedColumn());
				try{
					int iloscZmieniona = Integer.parseInt(tableSprzet.getCellEditor(tableSprzet.getSelectedRow(), tableSprzet.getSelectedColumn()).getCellEditorValue().toString());
					if(iloscZmieniona>0){
						sprzetyforma.get(tableSprzet.getSelectedRow()).setIloscSprzetu(iloscZmieniona);
						System.out.println("Powodzenie");
					}
					else{
						JOptionPane.showMessageDialog(null, "Ilość sztuk wybranej pozycji musi być dodatnia!", "Błąd wprowadzania danych", JOptionPane.ERROR_MESSAGE);
						System.out.println("Błąd - wartość niepoprawna");
					}
					
				}catch(NumberFormatException e){
					JOptionPane.showMessageDialog(null, "Ilość sztuk wybranej pozycji musi być liczbą!", "Błąd wprowadzania danych", JOptionPane.ERROR_MESSAGE);
		...
		//akcja dla przycisku Dodaj
		btnAccept.addActionListener(new ActionListener(){
			@Override
			public void actionPerformed(ActionEvent event) {
				EntityZamowienie zamowienie = formValidate();
				try{
					if(!zamowienie.equals(null)){
					...
					System.out.println("Zamówienie nie zostało dodane!");
				}
			}
		});
    ...
    private EntityZamowienie formValidate(){
    	...
    	if(wykladowca.equals(null)){
    		JOptionPane.showMessageDialog(null, "Nie wybrano wykładowcy!", "Błąd wprowadzania danych", JOptionPane.ERROR_MESSAGE);
    		return null;}
    	
    	EntitySala sala = (EntitySala) cbSala.getSelectedItem();
    	if(sala.equals(null)){
    		JOptionPane.showMessageDialog(null, "Nie wybrano sali!", "Błąd wprowadzania danych", JOptionPane.ERROR_MESSAGE);
    		return null;}
		...
		if(cbRozpoczecie.getSelectedIndex()>cbZakonczenie.getSelectedIndex()){
			JOptionPane.showMessageDialog(null, "Godzina rozpoczęcia nie może być późniejsza od godziny rozpoczęcia!", "Błąd wprowadzania danych", JOptionPane.ERROR_MESSAGE);
			return null;
		}
		if(chckbxZamwienieStale.isSelected()){
			try {
				...
				JOptionPane.showMessageDialog(null, "Wystąpił błąd podczas parsowania daty!", "Błąd wewnętrzny", JOptionPane.ERROR_MESSAGE);
				return null;
			}
			if(!chckStalePn.isSelected()&&
			   !chckStaleWt.isSelected()&&
			   !chckStaleSr.isSelected()&&
			   !chckStaleCz.isSelected()&&
			   !chckStalePt.isSelected()&&
			   !chckStaleSb.isSelected()&&
			   !chckStaleNd.isSelected()){
				JOptionPane.showMessageDialog(null, "Przy zamówieniu stałym przynajmniej jeden dzień tygodnia musi zostać zaznaczony!", "Błąd wewnętrzny", JOptionPane.ERROR_MESSAGE);
				return null;
			}
			...
			if(!dpStart.getJFormattedTextField().getText().equals(null)){
				try {
					...
					if(comp>=0){}
						...
						JOptionPane.showMessageDialog(null, "Nie można ustawić zamówienia na wcześniejszą datę!", "Błąd wprowadzania danych", JOptionPane.ERROR_MESSAGE);
						return null;
					}
				} catch (ParseException e) {
					dataRozpoczecia = null;
					dataZakonczenia = null;
					JOptionPane.showMessageDialog(null, "Data nie może być pusta!", "Błąd wprowadzania danych", JOptionPane.ERROR_MESSAGE);
					return null;
				}
			}else{
				JOptionPane.showMessageDialog(null, "Nie wprowadzono daty, lub są one niepoprawne!", "Błąd wprowadzania danych", JOptionPane.ERROR_MESSAGE);
				return null;}
		}
    	
		if(sprzetyforma.size()==0){
			JOptionPane.showMessageDialog(null, "Lista sprzętów do zamówienia jest pusta!", "Błąd wprowadzania danych", JOptionPane.ERROR_MESSAGE);
    		return null;
    	}else{
    		for(EntityZamowienieSprzetAlloc sprzet : sprzetyforma){
    			if(sprzet.getIloscSprzetu()==0){
    				JOptionPane.showMessageDialog(null, "Ilość sprzętu dla "+sprzet.getSprzet().getNazwa()+" nie może być zerowa!", "Błąd wprowadzania danych", JOptionPane.ERROR_MESSAGE);
    				return null;
    			}
    		}
    	}
		...
		if(tfRodzajZajec.getText().equals(null)){
			JOptionPane.showMessageDialog(null, "Rodzaj zajęć nie może być pusty!", "Błąd wprowadzania danych", JOptionPane.ERROR_MESSAGE);
			return null;
		}
		
		if(tpUwagi.getText().equals(null)){
			JOptionPane.showMessageDialog(null, "Uwagi nie mogą być puste!", "Błąd wprowadzania danych", JOptionPane.ERROR_MESSAGE);
			return null;
		}
    	...
    	if(chckbxZamwienieStale.isSelected()){
        	...
        	zamowienie.setZamowienieStale(new EntityZamowienieStale(chckStalePn.isSelected(),chckStaleWt.isSelected(),chckStaleSr.isSelected(),chckStaleCz.isSelected(),chckStalePt.isSelected(),chckStaleSb.isSelected(),chckStaleNd.isSelected()));
    	}else{
